...
const Login = () => {
    ...
    return (
        <div className="h-screen bg-[#F0F7F6] flex items-center justify-center p-4 font-sans overflow-hidden">
            <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-teal-500/5 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2" />
            ...
            <main className="w-full max-w-6xl flex bg-white rounded-[40px] shadow-[0_32px_80px_rgba(0,0,0,0.08)] border border-white/20 overflow-hidden h-[90vh] max-h-[850px] relative z-10 backdrop-blur-sm">
                ...
                <div className="hidden lg:flex w-[38%] flex-col justify-between p-12 bg-gradient-to-br from-[#164E44] via-[#2D7D6F] to-[#164E44] text-white relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="space-y-6">
                            <div className="space-y-1">
                                <span className="px-3 py-0.5 bg-teal-400/20 rounded-full text-[9px] font-black uppercase tracking-[0.2em] text-teal-200 border border-teal-400/20">Enterprise Grade</span>
                                <h1 className="text-4xl font-black leading-[1.1] tracking-tight">
                                    Secure <br />
                                    <span className="text-teal-300">Auth Portal</span>
                                </h1>
                            </div>
                            <p className="text-teal-50/70 text-sm leading-relaxed max-w-xs font-medium">
                        ...
                        <div className="p-5 bg-white/5 rounded-[28px] border border-white/10 backdrop-blur-md">
                            <div className="flex items-center gap-4 mb-3">
                                <div className="flex -space-x-3">
                                    {[1, 2, 3].map(idx => (
                                        <img key={idx} className="w-8 h-8 rounded-full border-2 border-[#2D7D6F] shadow-lg" src={`https://i.pravatar.cc/100?u=doc-${idx}`} alt="user" />
                                    ))}
                                </div>
                            ...
                            <div className="space-y-0.5">
                                ...
                                <p className="text-[10px] font-bold text-teal-300/60 uppercase tracking-widest">Global Medical Compliance</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <header className="p-6 lg:px-12 flex items-center justify-between shrink-0">
                        ...
                        <div className="hidden lg:block h-1 w-24 bg-slate-50 rounded-full overflow-hidden">
                            <motion.div
                                ...
                                className="h-full w-1/2 bg-teal-500/20"
                            />
                        </div>
                                ...
                                {roles.map((role) => (
                                    <button
                                        ...
                                        className={`group relative flex flex-col items-center p-4 rounded-[24px] border-2 transition-all duration-500 ${selectedRole === role.id
                                            ? 'border-teal-500 bg-teal-50/40 shadow-[0_15px_30px_rgba(20,184,166,0.1)]'
                                            : 'border-slate-50 bg-slate-50/50 hover:border-slate-200 hover:bg-white'
                                            }`}
                                    >
                                        <div className={`w-11 h-11 rounded-xl flex items-center justify-center mb-2 transition-all duration-500 ${selectedRole === role.id ? 'bg-teal-600 text-white shadow-xl scale-110 rotate-3' : 'bg-white text-slate-400 shadow-sm group-hover:scale-110'
                                            }`}>
                                            <role.icon size={22} strokeWidth={2.5} />
                                        </div>
                                        <span className={`text-[8px] font-black uppercase tracking-[0.15em] ${selectedRole === role.id ? 'text-teal-700' : 'text-slate-500'
                                            }`}>
                                            {selectedRole === role.id ? `${role.id} active` : role.id}
                                        </span>
                                        {selectedRole === role.id && (
                                            <motion.div layoutId="activeRole" className="absolute inset-0 border-2 border-teal-500 rounded-[24px] pointer-events-none" />
                                        )}
                                    </button>
                                ))}
                            </div>
                                    ...
                                    >
                                        <div className="space-y-1.5">
                                            ...
                                            <div className="relative group">
                                                <div className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center border-r border-slate-200 group-focus-within:border-teal-500 transition-colors pr-2">
                                                    <Mail className="w-3.5 h-3.5 text-slate-300 group-focus-within:text-teal-600 transition-colors" />
                                                </div>
                                                <input
                                                    type="text"
                                                    placeholder={
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <div className="relative group">
                                                <div className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center border-r border-slate-200 group-focus-within:border-teal-500 transition-colors pr-2">
                                                    <Lock className="w-3.5 h-3.5 text-slate-300 group-focus-within:text-teal-600 transition-colors" />
                                                </div>
                                                <input
                                                    type="password"
                                                    placeholder="••••••••••••"
                                        ...
                                        <div className="flex items-center justify-between text-[10px] font-black text-slate-400 px-1">
                                            <label className="flex items-center gap-2 cursor-pointer group hover:text-slate-900 transition-colors">
                                                <div className="relative flex items-center justify-center w-4 h-4 border-2 border-slate-200 rounded-md group-hover:border-teal-500 transition-colors">
                                                    <input type="checkbox" className="sr-only peer" />
                                                    <div className="w-2 h-2 bg-teal-600 rounded-sm opacity-0 peer-checked:opacity-100 transition-opacity" />
                                                </div>
                                                Remember Authority
                                            </label>
                                            <a href="#" className="text-teal-600 hover:text-teal-700 hover:underline underline-offset-4 decoration-2">Lost Credentials?</a>
                                        </div>
                                        ...
                                        >
                                            ...
                                            <div className="w-6 h-6 rounded-lg bg-white/10 flex items-center justify-center group-hover:bg-teal-500 transition-colors">
                                                <ArrowRight size={14} />
                                            </div>
                                        </button>
                                    </motion.div>
