...
const StaffAppointments = () => {
    ...
    const handleUpdateStatus = async (id, status) => {
        try {
            await api.put(`/appointments/update-status/${id}`, { status });
            setMessage({
                type: 'success',
                text: `Session ${status === 'confirmed' ? 'authorized' : 'deferred'} successfully!`
            });
            fetchAppointments();
                            ...
                            {loading ? (
                                ...
                                filteredAppointments.map((appt, idx) => (
                                    <motion.tr
                                        ...
                                        key={appt._id}
                                        className="hover:bg-[#F8FBFA] transition-all group group"
                                    >
